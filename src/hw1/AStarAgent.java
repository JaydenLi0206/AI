  ...
  */
    ...
    private boolean shouldReplanPath(State.StateView state, History.HistoryView history, Stack<MapLocation> currentPath)
    {
    	MapLocation enemyloc = null;
    	 //if the path is empty, then we don't need to replan
    	if(currentPath.isEmpty()){
    		return false;
    	}
            
        // enemy footman is not dead
    	if (enemyFootmanID != -1){
    		enemyloc = new MapLocation(state.getUnit(enemyFootmanID).getXPosition(), state.getUnit(enemyFootmanID).getYPosition(), null);
    	}
    	else{
    	...
    	for (MapLocation l : currentPath){
            //check if footman is in the current path, if so, replan
    		if (l.equals(enemyloc)){
    			return true;
    		}
    	}
        // if the enemy is not in the current path, don't replan
        return false;
    }
    ...
    {
    	//set up open and closed lists and the path stack
    	ArrayList<MapLocation> open = new ArrayList<MapLocation>();
    	ArrayList<MapLocation> close = new ArrayList<MapLocation>();
    	Stack<MapLocation> path = new Stack<MapLocation>();
        //add the start location to the open list
    	open.add(start);
        //while the open list is not empty
    	while(!open.isEmpty()) {
            //get the location on the open list with the lowest cost
    		MapLocation curr = open.get(0);
    		open.remove(curr);
    		close.add(curr);
            //if the location is the goal return the path

    		ArrayList<MapLocation> allneighbor=getAndCheckNeighbors(curr.g,curr,goal,xExtent,yExtent,enemyFootmanLoc, resourceLocations);
    		for(MapLocation neighbor:allneighbor) {
    			//Calculate the path cost 
    			float currcost = curr.f + 1;
                //if the location is the goal return the path
    			if(neighbor.equals(goal)) {
    				path=tracePath(start, neighbor,path);
    				path.remove(0);
    				return path;
    			}
                //if the location is already in the open list and the current path is cheaper
    			if(currcost<neighbor.f) {
                ...
                //if the location is not in the open or closed list add it to the open list
