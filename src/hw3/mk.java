...
public class mk extends Agent
{
	...
	public mk(int playerId, String[] args)
	{
		super(playerId);
		System.out.println("Constructed Opponent Agent, playerID: " + this.playernum);
		String streamerArgString = null;
		String paramFilePath = null;

		if(args.length < 3)
		{
			System.err.println("mk.mk [ERROR]: need to specify playerId, streamerArgString, paramFilePath");
			System.exit(-1);
		}
		...
		int numEpisodesToPlay = mk.NUM_TRAINING_EPISODES_IN_BATCH;
		...
		this.totalRewards = new ArrayList<Double>((int)this.NUM_EPISODES_TO_PLAY / mk.NUM_TRAINING_EPISODES_IN_BATCH);
		this.totalRewards.add(0.0);

		this.streamer = Streamer.makeDefaultStreamer(streamerArgString, this.getPlayerNumber());
		this.random = new Random(mk.RANDOM_SEED);
	...
	private Model initializeQFunction(boolean loadParams)
	{
		 ...
		 */
		int feature_dim = 4; // Adjusted: 4 input features instead of 64
		int hidden_dim1 = 128; // Example: 128 neurons in the first hidden layer
		int hidden_dim2 = 64; // Example: 64 neurons in the second hidden layer
		int hidden_dim3 = 32; // Example: 32 neurons in the third hidden layer
     ...
     // Holds the last turn number
    	...
    	for(DamageLog damageLog : history.getDamageLogs(lastTurnNumber)) {
    		...
    		if (verbose){
    	     ...
    	     if (damageLog.getDefenderController() == ENEMY_PLAYER_ID) {
		...
		for(DeathLog deathLog: history.getDeathLogs(lastTurnNumber)) {
			...
			if (deathLog.getController() == ENEMY_PLAYER_ID) {
				// Add the kill reward
                                        ...
                                        int atkUnitId, int tgtUnitId)
   {
	   /** TODO: complete me! **/
	    ...
	    Matrix featureVector = Matrix.zeros(1,4); // initilizing a row vector with X columns
...
//	    List<Integer> unitIds = state.getPlayerUnits(atkUnit.getOwner());
//	    int unitCount = unitIds.size();
//	    featureVector.set(0, 5, unitCount);
//
//	    // Add enemy count feature
//	    List<Integer> enemyIds = state.getOpponentUnits(atkUnit.getOwner());
//	    int enemyCount = enemyIds.size();
//	    featureVector.set(0, 6, enemyCount);
//
//	    // Add gold features
//	    int gold = state.getResourceAmount(atkUnit.getOwner(), ResourceType.GOLD);
//	    featureVector.set(0, 7, gold);
//
//	    // Add wood features
//	    int wood = state.getResourceAmount(atkUnit.getOwner(), ResourceType.WOOD);
//	    featureVector.set(0, 8, wood);
//
//	    // Add turn number feature
//	    featureVector.set(0, 9, state.getTurnNumber());
    ...
    private double calculateQValue(Matrix featureVec)
    {
        ...
        {
			System.err.println("mk.caculateQValue [ERROR]: error in either forward() or item()");
			e.printStackTrace();
			System.exit(-1);
		}
        return qValue;
    ...
    private int selectAction(StateView state, HistoryView history, int atkUnitId)
    {
    	...
    	if(this.getRandom().nextDouble() < mk.EPSILON)
    	{
    ...
    private Matrix getTDGroundTruth(StateView state, HistoryView history, int unitId) throws Exception
    {
    	...
    	return Matrix.full(1, 1, Rs + mk.GAMMA*maxQ); // output is always a scalar in active learning
    }
	...
	public Map<Integer, Action> initialStep(StateView state, HistoryView history)
	{
		...
		if(playerIds.size() != 2)
		{
			System.err.println("mk.initialStep [ERROR]: expected two players");
			System.exit(-1);
		}
    	...
    	for(Integer unitId : this.getMyUnitIds())
    	{
    				...
    				prevUnitActions.get(unitId).getFeedback().equals(ActionFeedback.FAILED))
    		{
    			if(state.getTurnNumber() > 0)
    			{
    				...
    				{
						System.err.println("mk.middleStep [ERROR]: problem updating gradients for transition on unitId=" + unitId);
						e.printStackTrace();
						System.exit(-1);
					}
    			}
	...
	public void terminalStep(StateView state, HistoryView history)
	{
		if(this.isTrainingEpisode())
		{
			...
			if((this.numTrainingEpisodesPlayed % mk.NUM_TRAINING_EPISODES_IN_BATCH) == 0)
			{
		...
		} else
		{
			...
			if((this.numTestEpisodesPlayedInBatch % mk.NUM_TESTING_EPISODES_IN_BATCH) == 0)
			{
				this.numTestEpisodesPlayedInBatch = -1;
				// calculate the average
				this.getTotalRewards().set(this.getTotalRewards().size()-1,
						this.getTotalRewards().get(this.getTotalRewards().size()-1) / mk.NUM_TRAINING_EPISODES_IN_BATCH);
    ...
    private void printTestData (List<Double> averageRewards)
    {
        ...
        {
            String gamesPlayed = Integer.toString(mk.NUM_TRAINING_EPISODES_IN_BATCH*(i+1));
